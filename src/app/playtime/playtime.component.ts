...
export class PlaytimeComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if(this.data_type === 'infantry'){
      ...
      this.data[0] += this.playtime.get('infiltrator')[this.index];
      this.data[1] += this.playtime.get('light assault')[this.index];
      this.data[2] += this.playtime.get('medic')[this.index];
      this.data[3] += this.playtime.get('engineer')[this.index];
      this.data[4] += this.playtime.get('heavy assault')[this.index];
      this.data[5] += this.playtime.get('max')[this.index];
    }
